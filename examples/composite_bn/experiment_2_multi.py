...
def run_example(file):
    ...
    complexity_FF = fitness_function.classical_K2 
    ...
    data_train_K2 , data_test_K2 = train_test_split(discretized_data_K2, test_size=0.2, shuffle = True, random_state=random_seed[number-1])
    ...
    for optimized_graph in optimized_graphs:
        ...
        write = Write()
        write.write_txt(vars_of_interest, path = os.path.join(parentdir, 'examples', 'results'), file_name = 'multi_results_' + file + '_run_' + str(number) + '.txt')
